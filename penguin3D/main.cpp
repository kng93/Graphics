...
void lighting()
{
    ...
    {
	memcpy(mat_specular, (float [4]){0.8f,0.8f,0.8f,1.0f}, 4*sizeof(float));
        memcpy(mat_diffuse, (float [4]){0.2f,0.2f,0.2f,1.0f}, 4*sizeof(float));
        memcpy(mat_ambient, (float [4]){0.3f,0.3f,0.3f,1.0f}, 4*sizeof(float));
        memcpy(mat_shininess, (float [1]){0.4 * 128}, sizeof(float));
    }
    else if (materialProp == MATTE)
    {
	memcpy(mat_specular, (float [4]){0.1f,0.1f,0.1f,1.0f}, 4*sizeof(float));
        memcpy(mat_diffuse, (float [4]){0.9f,0.9f,0.9f,1.0f}, 4*sizeof(float));
        memcpy(mat_ambient, (float [4]){0.3f,0.3f,0.3f,1.0f}, 4*sizeof(float));
        memcpy(mat_shininess, (float [1]){0.25 * 128}, sizeof(float));
    } 
...
void drawPenguin()
{
	...
	drawBody();
        ...
        // Draw the back arm
        glPushMatrix();
		// Translate and allow for movement of whole arm
                glTranslatef(-0.15,  0.8, -0.8);
		...
		// Move arm to rotation location	
		glPushMatrix();
                	glTranslatef( 0.15, -0.8, -0.85);
			drawArm();
		glPopMatrix();

		// Draw the back hand
		glPushMatrix();
                	// Move it to the correct position in reference to arm
                	glTranslatef(-0.15, -1.2, 0);
			// Move hand to rotation location then allow for rotation
			glRotatef(joint_ui_data->getDOF(Keyframe::R_ELBOW), 1.0, 0.0, 0.0);
                	glTranslatef(0.3, -0.2, -0.85);
			drawHand();
        	glPopMatrix();
        glPopMatrix();
	...
	glPushMatrix();
		...
		// Move arm to the rotational location
        	glPushMatrix();
			glTranslatef( 0.15, -0.8, -0.85);
			drawArm();
		glPopMatrix();
	...
	glPopMatrix();
		...
		// Draw the right eye
                ...
                // Draw the top beak
                glPushMatrix();
			glTranslatef(0.0, joint_ui_data->getDOF(Keyframe::BEAK)/2, 0.0);
                        glTranslatef(-1.15, 0.07, 0);
                        drawBeak();
                glPopMatrix();
		...
		// Move leg to the rotational location
		glPushMatrix();
...
void display(void)
{
	...
	// Draw the penguin with lighting
	lighting();
	glPushMatrix();
		...
		{
			// Change the material of the wireframe so it looks black
			setWireframeMat();
			glPolygonMode( GL_FRONT_AND_BACK, GL_LINE );
		}
...
// Draw the penguin body, centered at the current location
void drawBody()
{
	glBegin(GL_QUADS);
		// draw front face
		glNormal3f( 0.0,  0.0, 1.0);
                glVertex3f(-1.3, -1.5, 0.8);
                glVertex3f( 1.3, -1.5, 0.8);
                glVertex3f( 0.8,  1.5, 0.8);
                glVertex3f(-0.8,  1.5, 0.8);
...
// Draw the penguin head, centered at the current location
void drawHead()
{
	glBegin(GL_QUADS);
                // draw front face
                glNormal3f( 0.0,  0.0, 1.0);
		glVertex3f(-0.9, -0.6, 0.9);
                glVertex3f( 0.9, -0.6, 0.9);
...
// Draw the penguin beak, centered at the current location
void drawBeak()
{
        glBegin(GL_QUADS);
                // draw front face
                glNormal3f( 0.0,  0.0, 1.0);
		glVertex3f( -0.3,  0.07, 0.07);
                glVertex3f(  0.3,  0.07, 0.07);
                glVertex3f(  0.3, -0.07, 0.07);
                glVertex3f( -0.4, -0.07, 0.07);
...
// Draw the penguin arm, centered at the current location
void drawArm()
{
        glBegin(GL_QUADS);
                // draw front face
		glNormal3f( 0.0,  0.0, 1.0);
                ...
                glVertex3f( 0.3,  0.8, 0.9);
                glVertex3f( 0.0, -0.6, 0.9);
                glVertex3f(-0.55,-0.5, 0.9);

                // draw back face
		glNormal3f( 0.0,  0.0, -1.0);
                glVertex3f( 0.0, -0.6, 0.8);
                glVertex3f(-0.55,-0.5, 0.8);
                glVertex3f(-0.6,  1.0, 0.8);
                glVertex3f( 0.3,  0.8, 0.8);
...
// Draw the penguin hand, centered at the current location
void drawHand()
{
        glBegin(GL_QUADS);
                // draw front face
		glNormal3f( 0.0,  0.0, 1.0);
                glVertex3f(-0.5,  0.2, 0.9);
...
// Draw the penguin leg, centered at the current location
void drawLeg()
{
        glBegin(GL_QUADS);
                // draw front face
...
void drawFoot()
{
	...
	glEnd();
        ...
        glBegin(GL_QUADS);
                // draw the front side of the foot
		glNormal3f( -1.0,  0.0,  0.0);
                glVertex3f( -0.6,  0.0, -0.2);
                glVertex3f( -0.6,  0.0,  0.2);
                glVertex3f( -0.6, -0.05, -0.2);
                ...
                // draw the left side of the foot
		glNormal3f(  0.2,  0.0, -0.6);
                glVertex3f(  0.0,  0.0,  0.0);
                glVertex3f( -0.6,  0.0,  0.2);
                glVertex3f(  0.0, -0.05,  0.0);
                glVertex3f( -0.6, -0.05,  0.2);

                // draw the right side of the foot
		glNormal3f(  0.2,  0.0,  0.6);
                glVertex3f(  0.0,  0.0,  0.0);
                glVertex3f( -0.6,  0.0, -0.2);
                glVertex3f(  0.0, -0.05,  0.0);
